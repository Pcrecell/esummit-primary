...
const PandorasParadoxDashboard = () => {
  ...
  const handleSubmitCreate = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/hackathon/hackathon_registration`,
        {
          method: "POST",
          headers: {
  ...
  const handleAddMemberButton = async () => {
    ...
    try {
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/hackathon/add-member`,
        {
      ...
      setTeamInfo((prev) => ({
        ...
        members: [
          ...prev.members,
          { name: newTeammateName, elixir: newTeammateId },
        ],
      }));
  ...
  const handleRemoveMember = async (memberelixir) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/hackathon/remove-member`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    >
            ...
            {action === "registered" && (
              <div className="w-full bg-green-600/90 border-2 border-green-400/60 rounded-lg p-4 sm:p-6 backdrop-blur-md">
                ...
                <p className="font-mono text-white text-xs sm:text-sm leading-relaxed">
                  Your team{" "}
                  <span className="font-bold">{teamInfo.teamName}</span> is
      ...
      {!isRegistered && action === "details" && (
        <div className="pt-8 left-1/2 transform z-30">
          <button
            disabled={
              teamInfo.members.filter((member) => member.name !== "-").length <
              2
            }
